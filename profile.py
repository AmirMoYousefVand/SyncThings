    ...
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
                ...
                private_paste_hotkey = config.get("private_paste_hotkey", private_paste_hotkey)
                browser_hotkey = config.get("browser_hotkey", browser_hotkey)
        except Exception:
            pass

    if not avatar_b64 and not avatar_path:
        avatar_b64, mini_avatar_b64 = generate_random_avatar()
        save_profile(profile_name, avatar_path, avatar_b64, mini_avatar_b64, theme, lang, state, enable_size_limit, max_file_size_mb, enable_ext_limit, ext_mode, target_extensions, private_paste_hotkey, browser_hotkey)

    return profile_name, avatar_path, avatar_b64, theme, lang, state, enable_size_limit, max_file_size_mb, enable_ext_limit, ext_mode, target_extensions, private_paste_hotkey, browser_hotkey

def save_profile(profile_name, avatar_path, avatar_b64=None, mini_avatar_b64=None, theme="Light", lang="en", state="zoomed", enable_size_limit=False, max_file_size_mb=100, enable_ext_limit=False, ext_mode="exclude", target_extensions="", private_paste_hotkey="", browser_hotkey=""):
    ...
    config = {
        ...
        "max_file_size_mb": max_file_size_mb, "enable_ext_limit": enable_ext_limit, "ext_mode": ext_mode, "target_extensions": target_extensions, "private_paste_hotkey": private_paste_hotkey, "browser_hotkey": browser_hotkey
    }
    try:
